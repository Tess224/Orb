...
def get_token_liquidity_simple(token_address: str) -> Dict:
    """
    Fetches liquidity from Birdeye with simple fallback to market cap estimation.
    """
    ...
    # Fallback: estimate liquidity from market cap
    ...
    if market_cap:
        ...
        logger.info(f"✓ Estimated liquidity from MC ${market_cap:,.0f}: ${estimated_liquidity:,.0f}")
    ...
    # Final fallback
    logger.warning("No data available - using conservative default")
    return {
        'liquidity_usd': 25000,
        'market_cap_usd': None,
        'source': 'default'
...
def get_token_market_cap(token_address: str) -> Optional[float]:
    """
    Fetches market cap from Birdeye (used as fallback when liquidity unavailable).
    """
    try:
...
def estimate_liquidity_from_market_cap(market_cap: float) -> float:
    """
    Estimates liquidity from market cap using your empirical pump.fun data.
    """
    mapping = [
...
def classify_liquidity_tier(liquidity_usd: float) -> str:
    """
    Simple tier classification based on liquidity amount.
    """
...
def get_probe_sizes_for_liquidity(liquidity_usd: float, sol_price_usd: float) -> List[Dict]:
    """
    Determines probe sizes based purely on reported liquidity amount.
    """
    liquidity_strategies = [
        (20000, 40000, [50, 100, 250, 500], 1000),
        (40000, 75000, [75, 150, 300, 600], 1500),
        (75000, 150000, [150, 300, 600, 1200], 2500),
        (150000, 550000, [400, 800, 1600, 3200], 6000),
        (550000, 1000000, [600, 1200, 2400, 4800], 8000),
        (1000000, float('inf'), [1000, 2000, 4000, 8000], 12000)
    ]
    
    adaptive_sizes = None
    stress_size = None
    
    for liq_min, liq_max, adaptive, stress in liquidity_strategies:
        if liq_min <= liquidity_usd < liq_max:
            adaptive_sizes = adaptive
            stress_size = stress
            logger.info(f"Liquidity ${liquidity_usd:,.0f} falls in range ${liq_min:,.0f}-${liq_max:,.0f}")
            break
    
    if liquidity_usd < 20000:
        logger.warning(f"Liquidity ${liquidity_usd:,.0f} below minimum - using micro strategy")
        adaptive_sizes = [25, 50, 100, 250]
        stress_size = 500
    
    if adaptive_sizes is None:
        logger.warning("Could not determine probe strategy - using default")
    ...
    logger.info(f"Selected probe strategy: {[f'${p['usd_amount']}' for p in probe_configs]}")
