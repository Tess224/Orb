...
# ============================================================================
# PHASE 1: TOKEN INFORMATION SYSTEM
# ============================================================================

# Cache for storing token information to reduce API calls
# Structure: {"token_identifier": {"data": {...}, "timestamp": 123456}}
token_info_cache: Dict[str, Dict] = {}

# Cache duration: 60 seconds (prices change frequently)
TOKEN_INFO_CACHE_TTL = 60


def get_cached_token_info(identifier: str) -> Optional[Dict]:
    """
    Check if we have recent cached data for this token.
    
    Args:
        identifier: Either a token address or symbol
        
    Returns:
        Cached token data if it exists and isn't expired, otherwise None
    """
    if identifier not in token_info_cache:
        return None
    
    cached = token_info_cache[identifier]
    age = time.time() - cached['timestamp']
    
    # If cache is older than our TTL, consider it expired
    if age > TOKEN_INFO_CACHE_TTL:
        del token_info_cache[identifier]
        return None
    
    logger.info(f"💾 Cache hit for {identifier[:8]}... (age: {age:.1f}s)")
    return cached['data']


def set_cached_token_info(identifier: str, data: Dict):
    """
    Store token information in our cache with a timestamp.
    
    Args:
        identifier: Either a token address or symbol
        data: The token information to cache
    """
    token_info_cache[identifier] = {
        'data': data,
        'timestamp': time.time()
    }
    logger.info(f"💾 Cached token info for {identifier[:8]}...")


def fetch_token_from_devfun(identifier: str, lookup_type: str) -> Optional[Dict]:
    """
    Fetch token information from dev.fun API.
    
    This function replicates what the dev.fun SDK does in your frontend.
    We're making direct HTTP requests to their API endpoints.
    
    Args:
        identifier: Token address or symbol to look up
        lookup_type: Either 'address' or 'symbol'
        
    Returns:
        Dictionary with token information, or None if not found
    """
    try:
        # dev.fun has different endpoints for address vs symbol lookup
        if lookup_type == 'address':
            url = f"https://api.dev.fun/tokens/by-address/{identifier}"
        else:  # symbol
            url = f"https://api.dev.fun/tokens/by-symbol/{identifier}"
        
        logger.info(f"🔍 Fetching from dev.fun: {lookup_type}={identifier[:8]}...")
        
        response = requests.get(url, timeout=10)
        
        if response.status_code == 404:
            logger.warning(f"⚠️ Token not found on dev.fun: {identifier}")
            return None
        
        response.raise_for_status()
        data = response.json()
        
        # The dev.fun API returns different structures, we need to normalize it
        # Sometimes the token data is nested, sometimes it's at the root
        if isinstance(data, dict):
            # If there's a 'data' key, the actual token info is nested
            ...
            normalized = {
                ...
                'contractAddress': token_data.get('mint') or token_data.get('address'),
                'image': token_data.get('image_uri') or token_data.get('image'),
                'description': token_data.get('description'),
                'price': float(token_data.get('price', 0)),
                'marketcap': float(token_data.get('market_cap', 0)),
                'volumeIn24h': float(token_data.get('volume_24h', 0)),
                'priceChangeIn24h': float(token_data.get('price_change_24h', 0)),
            ...
            logger.info(f"✅ Successfully fetched token: {normalized['symbol']}")
            return normalized
        
        logger.error(f"❌ Unexpected response format from dev.fun: {type(data)}")
        return None
        
    except requests.exceptions.Timeout:
        logger.error(f"⏱️ Timeout fetching token from dev.fun: {identifier}")
        return None
    except requests.exceptions.RequestException as e:
        logger.error(f"❌ Error fetching token from dev.fun: {e}")
        return None
